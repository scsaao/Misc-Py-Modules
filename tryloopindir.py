...
def plottingNew(x,y,ey,periods,power,lwd=1.3,size=[10.,6.75],submarzins=[0.1,0.9,0.25],xlab1='TIME [s]', ylab1='y(t)',xlab2='$\omega$' + str('[s]'),ylab2='Power',xscale='log'):
    ...
    ax1.set_xlim(periods[0], 4.0*periods[-1])
    ax1.set_ylim(0.0,0.03)
    #    ax1.set_ylim(power.min()-((power.max()-power.min())*0.05), power.max()+((power.max()-power.min())*0.5))
...
with open(filelist) as input_file:
    for i, line in enumerate(input_file):
        ...
        #----------------------------------------------------------------
        print "The ASTROSAT source we are working for:  " + Source
        ...
        for j in range(len(paramsIn)-1):
            ...
            plottingNew(x=t,y=mag,ey=dy,periods=periods,power=power,lwd=1.3,size=[10.,6.75],submarzins=[0.1,0.9,0.25],xlab1='TIME [s]', ylab1=paramsIn[j + 1]" [Deg] ",xlab2='$2 \pi / \omega$ ' + str(' [s]'),ylab2=r'$P_{LS} (\omega)$',xscale='log')
